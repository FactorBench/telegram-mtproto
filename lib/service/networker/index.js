...
class NetworkerThread {
  constructor({
    ...
    this.checkLongPoll = _asyncToGenerator(function* () {
      ...
      if (_this.checkLongPollAfterDcCond(isClean, baseDc))
        ...
        return;
      return _this.sendLongPoll();
    });
    this.sendLongPoll = _asyncToGenerator(function* () {
      var maxWait = 25000;
      _this.longPollPending = (0, _timeManager.tsNow)() + maxWait;
      // console.log('Set lp', this.longPollPending, tsNow())

      yield _this.wrapMtpCall('http_wait', {
        max_delay: 500,
        wait_after: 150,
        max_wait: maxWait
      }, {
      ...
      delete _this.longPollPending;
      return _this.checkLongPoll();
    });

    function* _ref(event) {
      log([`Check connection`])('%O', event);
      _smartTimeout2.default.cancel(_this.checkConnectionPromise);

      var serializer = _this.Serialization({ mtproto: true });
      var pingID = [(0, _bin.nextRandomInt)(0xFFFFFFFF), (0, _bin.nextRandomInt)(0xFFFFFFFF)];

      serializer.storeMethod('ping', { ping_id: pingID });

      var pingMessage = new _netMessage.NetMessage(_this.generateSeqNo(true), serializer.getBytes());
      ...
      _this.checkConnectionPromise = (0, _smartTimeout2.default)(_this.checkConnection, delay);
      _this.checkConnectionPeriod = Math.min(60, _this.checkConnectionPeriod * 1.5);
    }

    this.checkConnection = (() => {
      var _ref3 = _asyncToGenerator(_ref);
    ...
    this.performSheduledRequest = _asyncToGenerator(function* () {
      ...
      if (_this.offline || akStopped) {
        log(`Cancel sheduled`)(``);
        return false;
      }
      delete _this.nextReq;
      if (_this.pendingAcks.length) {
        var ackMsgIDs = [];
        for (var _iterator = _this.pendingAcks, _isArray = Array.isArray(_iterator), _i = 0, _iterator = _isArray ? _iterator : _iterator[Symbol.iterator]();;) {
          var _ref5;
      ...
      for (var _iterator2 = _this.state.pendingIterator(), _isArray2 = Array.isArray(_iterator2), _i2 = 0, _iterator2 = _isArray2 ? _iterator2 : _iterator2[Symbol.iterator]();;) {
        var _ref6;
        ...
        var [messageID, value] = _ref6;

        if (value && value < currentTime) continue;
        _this.state.deletePending(messageID);
        if (!_this.state.hasSent(messageID)) continue;
        message = _this.state.getSent(messageID);
      ...
      if (hasApiCall && !hasHttpWait) {
        ...
        messages.push(new _netMessage.NetMessage(_this.generateSeqNo(), serializer.getBytes()));
      }
      ...
      if (messages.length > 1) {
        var container = _this.Serialization({ mtproto: true, startMaxLength: messagesByteLen + 64 });
        ...
        message = new _netMessage.NetContainer(_this.generateSeqNo(true), container.getBytes(true), innerMessages);
      ...
      _this.state.addSent(message);

      _this.pendingAcks = []; //TODO WTF,he just clear and forget them at all?!?
      if (lengthOverflow || singlesCount > 1) _this.sheduleRequest();
      ...
      try {
        ...
        _this.processMessage(response.response, response.messageID, response.sessionID);
        ...
        for (var _iterator4 = noResponseMsgs, _isArray4 = Array.isArray(_iterator4), _i4 = 0, _iterator4 = _isArray4 ? _iterator4 : _iterator4[Symbol.iterator]();;) {
          ...
          if (_this.state.hasSent(msgID)) {
            var _msg = _this.state.getSent(msgID);
            _this.state.deleteSent(_msg);
            _msg.deferred.resolve();
          }
        }_this.checkConnectionPeriod = Math.max(1.1, Math.sqrt(_this.checkConnectionPeriod));

        return _this.checkLongPoll(); //TODO Bluebird warning here
      } catch (error) {
        ...
        for (var _iterator6 = noResponseMsgs, _isArray6 = Array.isArray(_iterator6), _i6 = 0, _iterator6 = _isArray6 ? _iterator6 : _iterator6[Symbol.iterator]();;) {
          var _ref10;
    ...
    function* _ref2(message, options = {}) {
      // console.log(dTime(), 'Send encrypted'/*, message*/)
      // console.trace()
      var data = _this.Serialization({ startMaxLength: message.body.length + 64 });
      var dataBox = data.writer;
      _tl.WriteMediator.intBytes(dataBox, _this.serverSalt, 64, 'salt');
      _tl.WriteMediator.intBytes(dataBox, _this.sessionID, 64, 'session_id');
      ...
      var url = _this.chooseServer(_this.dcID, _this.upload);
      ...
      var keyIv = yield _this.getMsgKeyIv(msgKey, true);
      var encryptedBytes = yield _crypto2.default.aesEncrypt(bytes, keyIv[0], keyIv[1]);

      var request = _this.Serialization({ startMaxLength: encryptedBytes.byteLength + 256 });
      var requestBox = request.writer;
      _tl.WriteMediator.intBytes(requestBox, _this.authKeyID, 64, 'auth_key_id');
      ...
      try {
        ...
        var result = yield fetch(url, _request);
        var _data = new Uint8Array((yield result.arrayBuffer())).buffer;
        return result.status === 404 || !_data || !_data.byteLength ? _bluebird2.default.reject(new _error.ErrorBadResponse(url, _data)) : { data: _data };
      } catch (error) {
        return _bluebird2.default.reject(new _error.ErrorBadResponse(url, error));
      }
    }

    this.sendEncryptedRequest = (() => {
      var _ref11 = _asyncToGenerator(_ref2);
  ...
  updateSentMessage(sentMessageID) {
    ...
    if (sentMessage instanceof _netMessage.NetContainer) {
      var newInner = [];
      for (var _iterator7 = sentMessage.inner, _isArray7 = Array.isArray(_iterator7), _i7 = 0, _iterator7 = _isArray7 ? _iterator7 : _iterator7[Symbol.iterator]();;) {
        var _ref12;
  ...
  pushResend(messageID, delay) {
    ...
    var sentMessage = this.state.getSent(messageID);
    if (sentMessage instanceof _netMessage.NetContainer) {
      for (var _iterator8 = sentMessage.inner, _isArray8 = Array.isArray(_iterator8), _i8 = 0, _iterator8 = _isArray8 ? _iterator8 : _iterator8[Symbol.iterator]();;) {
        var _ref13;
        ...
        this.state.setPending(_msg3, value);
      }
  ...
  getMsgKeyIv(msgKey, isOut) {
    ...
    return _asyncToGenerator(function* () {
      var authKey = _this2.authKeyUint8;
      var x = isOut ? 0 : 8;
  ...
  parseResponse(responseBuffer) {
    ...
    return _asyncToGenerator(function* () {
      ...
      var deserializerRaw = _this3.Deserialization(responseBuffer);

      var authKeyID = deserializerRaw.fetchIntBytes(64, 'auth_key_id');
      if (!(0, _bin.bytesCmp)(authKeyID, _this3.authKeyID)) {
      ...
      var keyIv = yield _this3.getMsgKeyIv(msgKey, false);
      var dataWithPadding = yield _crypto2.default.aesDecrypt(encryptedData, keyIv[0], keyIv[1]);
      // console.log(dTime(), 'after decrypt')
      var deserializer = _this3.Deserialization(dataWithPadding, { mtproto: true });
      ...
      var isInvalidSession = !(0, _bin.bytesCmp)(sessionID, _this3.sessionID) && (!_this3.prevSessionID ||
      //eslint-disable-next-line
      !(0, _bin.bytesCmp)(sessionID, _this3.prevSessionID));
      if (isInvalidSession) {
        console.warn('Sessions', sessionID, _this3.sessionID, _this3.prevSessionID);
      ...
      var deserializerOptions = getDeserializeOpts(_this3.getMsgById);
      var deserializerData = _this3.Deserialization(buffer, deserializerOptions);
  ...
  cleanupSent() {
    ...
    for (var _iterator9 = this.state.sentIterator(), _isArray9 = Array.isArray(_iterator9), _i9 = 0, _iterator9 = _isArray9 ? _iterator9 : _iterator9[Symbol.iterator]();;) {
      var _ref14;
      ...
      if (message.notContentRelated && !this.state.hasPending(msgID))
        ...
        for (var _iterator10 = message.inner, _isArray10 = Array.isArray(_iterator10), _i10 = 0, _iterator10 = _isArray10 ? _iterator10 : _iterator10[Symbol.iterator]();;) {
          var _ref15;
  ...
  processMessage(message, messageID, sessionID) {
    ...
    switch (message._) {
      case 'msg_container':
        {
          for (var _iterator11 = message.messages, _isArray11 = Array.isArray(_iterator11), _i11 = 0, _iterator11 = _isArray11 ? _iterator11 : _iterator11[Symbol.iterator]();;) {
            var _ref16;
      ...
      case 'msgs_state_info':
        {
          ...
          if (spliceCond) {
            for (var _iterator12 = this.lastResendReq.resend_msg_ids, _isArray12 = Array.isArray(_iterator12), _i12 = 0, _iterator12 = _isArray12 ? _iterator12 : _iterator12[Symbol.iterator]();;) {
              var _ref17;
