...
var _initialiseProps = function () {
  ...
  function* _ref10(message, options = {}) {
    ...
    try {
      ...
      if (_this3.platform == 'web') {
        result = yield _http.httpClient.post(url, requestData, options);
      } else {
        var tcpClient = new _tcp2.default({ url });
        result = yield tcpClient.post(requestData);
      }
      console.log('>>> NETWORKER result:', result.data, '<<<');
      return !result.data || !result.data.byteLength ? _bluebird2.default.reject(new _error.ErrorBadResponse(url, result)) : result;
    } catch (error) {
      console.log('>>> NETWORKER ERROR:', error, '<<<');
...
var NetworkerFabric = exports.NetworkerFabric = (appConfig, { Serialization, Deserialization }, storage, emit, platform) => chooseServer => (dc, authKey, serverSalt, options = {}) => new NetworkerThread({
  appConfig,
  chooseServer,
  Serialization,
  Deserialization,
