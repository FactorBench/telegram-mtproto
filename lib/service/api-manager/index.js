...
class ApiManager {
  constructor(config, tls, netFabric, { on, emit }) {
    ...
    function* _ref3(dcID, options = {}) {
      ...
      var isUpload = options.fileUpload || options.fileDownload;
      ...
      console.log('[MtpGetNetworker] cached =', (0, _has2.default)(dcID, cache));
    ...
    function* _ref6(method, params, options = {}) {
      ...
      var cfg = {
        ...
        netOpts: options,
        fixupDc: _this.fixupDc.bind(_this)
      };
  ...
  initConnection() {
    ...
    return _asyncToGenerator(function* () {
      console.log('[initConnection] check exists any networker:', !isAnyNetworker(_this2));
      console.log('[initConnection] networkers:', _this2.cache);
      if (!isAnyNetworker(_this2)) {
        var storedBaseDc = yield _this2.storage.get('dc');
        console.log('[initConnection] stored default dc id:', storedBaseDc, '. While default from settings:', _this2.baseDcID);
